...
def main() -> int:
    parser = argparse.ArgumentParser(description="Install the Swarm Checkup skill into Codex's local skills directory.")
    parser.add_argument("--dest", default=str(Path(os.environ.get("CODEX_HOME", Path.home() / ".codex")) / "skills"))
    args = parser.parse_args()

    repo_root = Path(__file__).resolve().parents[1]
    source = repo_root / "skills" / "swarm-checkup"
    dest_root = Path(args.dest).expanduser().resolve()
    dest = dest_root / "swarm-checkup"
